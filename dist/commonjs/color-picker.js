...
(function ($) {
	var ColorPicker = (function () {
		var ids = {},
		    ...
		    fillRGBAFields = function fillRGBAFields(hsba, cal) {
			var rgba = HSBAToRGBA(hsba);
			$(cal).data('colorpicker').fields.eq(1).val(Math.round(rgba.r)).end().eq(2).val(Math.round(rgba.g)).end().eq(3).val(Math.round(rgba.b)).end().eq(4).val(Math.round(rgba.a)).end();
		},
		    fillHSBAFields = function fillHSBAFields(hsba, cal) {
			$(cal).data('colorpicker').fields.eq(5).val(Math.round(hsba.h)).end().eq(6).val(Math.round(hsba.s)).end().eq(7).val(Math.round(hsba.b)).end().eq(8).val(Math.round(hsba.a)).end();
		},
		    fillHexFields = function fillHexFields(hsba, cal) {
			$(cal).data('colorpicker').fields.eq(0).val(HSBAToHex(hsba)).end();
		},
		    setSelector = function setSelector(hsba, cal) {
		    ...
		    setHue = function setHue(hsba, cal) {
			...
			change.apply(ev.data.cal.data('colorpicker').fields.eq(5).val(parseInt(360 * (150 - Math.max(0, Math.min(150, ev.pageY - ev.data.y))) / 150, 10)).get(0), [ev.data.preview]);
			return false;
		},
		    ...
		    downAlpha = function downAlpha(ev) {
			var current = {
				cal: $(this).parent(),
				x: $(this).offset().left
			};
		    ...
		    moveAlpha = function moveAlpha(ev) {
			change.apply(ev.data.cal.data('colorpicker').fields.eq(8).val(parseInt(100 * Math.max(0, Math.min(150, ev.pageX - ev.data.x)) / 150, 10)).get(0), [ev.data.preview]);
			return false;
		},
		    upAlpha = function upAlpha(ev) {
			fillRGBAFields(ev.data.cal.data('colorpicker').color, ev.data.cal.get(0));
			fillHexFields(ev.data.cal.data('colorpicker').color, ev.data.cal.get(0));
			...
			change.apply(ev.data.cal.data('colorpicker').fields.eq(7).val(parseInt(100 * (150 - Math.max(0, Math.min(150, ev.pageY - ev.data.pos.top))) / 150, 10)).end().eq(6).val(parseInt(100 * Math.max(0, Math.min(150, ev.pageX - ev.data.pos.left)) / 150, 10)).get(0), [ev.data.preview]);
			return false;
		},
		    ...
		    HexToHSBA = function HexToHSBA(hex) {
			return RGBAToHSBA(HexToRGBA(hex));
		},
		    RGBAToHSBA = function RGBAToHSBA(rgba) {
			var hsba = {
				h: 0,
				s: 0,
				b: 0,
		    ...
		    HSBAToRGBA = function HSBAToRGBA(hsba) {
			var rgba = {};
		    ...
		    HSBAToHex = function HSBAToHex(hsba) {
			return RGBAToHex(HSBAToRGBA(hsba));
		},
		    HSBAToCSSRGBA = function HSBAToCSSRGBA(hsba) {
			var rgba = HSBAToRGBA(hsba);
